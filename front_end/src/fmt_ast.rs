    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            EndCondition::UntilBoolRep {
                ...
                logic: logic_bin_op,
                reps: repititions,
            } => &format!("until {} {} {}", bool_expr, logic_bin_op, repititions),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            AggregatePlayer::OwnerOfCardPostion { card_position } => {
                &format!("owner of {}", card_position)
            }
            AggregatePlayer::OwnerOfMemory { extrema, memory } => {
                &format!("owner of {} {}", extrema, memory)
            }
        };
        f.write_str(s)
    }
...
impl fmt::Display for PlayerExpr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            PlayerExpr::Literal { name } => &format!("P:{}", name),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            QueryInt::IntCollectionAt {
                int_collection,
                int_expr,
            } => &format!("{}[{}]", int_collection, int_expr),
        };
        f.write_str(s)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            AggregateInt::SumOfIntCollection { int_collection } => {
                &format!("sum({})", int_collection)
            }
            AggregateInt::SumOfCardSet { card_set, pointmap } => {
                &format!("sum of {} using {}", card_set, pointmap)
            }
            AggregateInt::ExtremaCardset {
                extrema,
                card_set,
                pointmap,
            } => &format!("{} of {} using {}", extrema, card_set, pointmap),
            AggregateInt::ExtremaIntCollection {
                extrema,
                int_collection,
            } => &format!("{}({})", extrema, int_collection),
        };
        f.write_str(s)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            QueryString::StringCollectionAt {
                string_collection,
                int_expr,
            } => &format!("{}[{}]", string_collection, int_expr),
        };
        f.write_str(s)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            AggregateBool::Compare {
                cmp_bool: compare_bool,
            } => &format!("{}", compare_bool),
            AggregateBool::CardSetEmpty { card_set } => &format!("{} empty", card_set),
            AggregateBool::CardSetNotEmpty { card_set } => &format!("{} not empty", card_set),
...
impl fmt::Display for TeamExpr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            TeamExpr::Literal { name } => &format!("T:{}", name),
            TeamExpr::Aggregate {
...
impl fmt::Display for QueryCardPosition {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            QueryCardPosition::At { location, int_expr } => &format!("{}[{}]", location, int_expr),
            QueryCardPosition::Top { location } => &format!("top({})", location),
            QueryCardPosition::Bottom { location } => &format!("bottom({})", location),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for AggregateCardPosition {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            AggregateCardPosition::ExtremaPointMap {
                extrema,
                card_set,
                pointmap,
            } => &format!("{} of {} using points {}", extrema, card_set, pointmap),
            AggregateCardPosition::ExtremaPrecedence {
                extrema,
                card_set,
                precedence,
...
impl fmt::Display for CardPosition {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            CardPosition::Query {
                query: query_card_position,
            } => &format!("{}", query_card_position),
            CardPosition::Aggregate {
                aggregate: aggregate_card_position,
...
impl fmt::Display for Group {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            Group::NotCombo { combo, groupable } => &format!("not {} in {}", combo, groupable),
            Group::Combo { combo, groupable } => &format!("{} in {}", combo, groupable),
            Group::CardPosition { card_position } => &format!("{}", card_position),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for AggregateFilter {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            AggregateFilter::Same { key } => &format!("same {}", key),
            AggregateFilter::Distinct { key } => &format!("distinct {}", key),
            ...
            AggregateFilter::KeyIsString {
                key,
                string: string_expr,
            } => &format!("{} is {}", key, string_expr),
            AggregateFilter::KeyIsNotString {
                key,
                string: string_expr,
...
impl fmt::Display for Collection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            Collection::CardSet { card_set } => &format!("cards {}", card_set),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for AggregatePlayerCollection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            AggregatePlayerCollection::Quantifier { quantifier: q } => &format!("{}", q),
        };
        f.write_str(s)
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            PlayerCollection::Aggregate {
                aggregate: aggregate_player_collection,
            } => &format!("{}", aggregate_player_collection),
            PlayerCollection::Runtime {
...
impl fmt::Display for SetUpRule {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            SetUpRule::CreateMemory { memory, owner } => &format!("memory {} on {}", memory, owner),
...
impl fmt::Display for ActionRule {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            ActionRule::ResetMemory { memory } => &format!("reset {}", memory),
            ActionRule::CycleAction {
                player: player_expr,
            } => &format!("cycle to {}", player_expr),
            ActionRule::BidAction { quantitiy } => &format!("bid {}", quantitiy),
            ActionRule::BidMemoryAction {
                memory,
                quantity,
                owner,
            } => &format!("bid {} on {} of {}", quantity, memory, owner),
            ActionRule::EndAction { end_type } => &format!("end {}", end_type),
            ActionRule::DemandAction { demand_type } => &format!("demand {}", demand_type),
            ActionRule::DemandMemoryAction {
                demand_type,
                memory,
            } => &format!("demand {} as {}", demand_type, memory),
            ActionRule::Move { move_type } => &format!("{}", move_type),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for MoveType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            MoveType::Deal { deal: deal_move } => &format!("{}", deal_move),
            MoveType::Exchange {
                exchange: exchange_move,
            } => &format!("{}", exchange_move),
            MoveType::Classic {
                classic: classic_move,
            } => &format!("{}", classic_move),
            MoveType::Place { token: token_move } => &format!("{}", token_move),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for TokenMove {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            TokenMove::Place {
                token,
                from_loc: token_loc_expr,
                to_loc: token_loc_expr1,
            } => &format!(
                "place {} from {} to {}",
                token, token_loc_expr, token_loc_expr1
            ),
            TokenMove::PlaceQuantity {
                ...
                "place {} {} from {} to {}",
                quantity, token, token_loc_expr, token_loc_expr1
            ),
        };
        f.write_str(s)
    }
...
impl fmt::Display for TokenLocExpr {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            TokenLocExpr::Groupable { groupable } => &format!("{}", groupable),
...
impl fmt::Display for ScoringRule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ScoringRule::ScoreRule { score_rule } => &format!("{}", score_rule),
            ScoringRule::WinnerRule { winner_rule } => &format!("{}", winner_rule),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for WinnerRule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            WinnerRule::Winner { players } => &format!("winner is {}", players),
...
impl fmt::Display for GameRule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            GameRule::SetUp { setup } => &format!("{}", setup),
            GameRule::Action { action } => &format!("{}", action),
            GameRule::Scoring { scoring } => &format!("{}", scoring),
        };
        f.write_str(s)
    }
}
...
impl fmt::Display for FlowComponent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            FlowComponent::Conditional { conditional } => &format!("{}", conditional),
            FlowComponent::TriggerRule { trigger_rule } => &format!("{}", trigger_rule),
        };
        f.write_str(s)
    }
}
